...
#define WMR_CONFIG "/etc/wmr.conf"
...
int main(int argc, char **argv)
{
    ...
    string wmrconfig = WMR_CONFIG;  // location of wmr928's config file
    ...
    for (int argpos = 1; argpos < argc; ++argpos)
    {
                  ...
                  !strcmp(argv[argpos],"--logfile"))
        {
            ...
            else
            {
                ...
                return EXIT_INVALID_CMDLINE;
                  ...
                  !strcmp(argv[argpos],"--wmrconfig"))
        {
            if (argc > argpos)
            {
                wmrconfig = argv[argpos+1];
                if (wmrconfig[0] == '-')
                {
                    cerr << "Invalid or missing argument to -c/--wmrconfig option\n";
                    return EXIT_INVALID_CMDLINE;
                }
            ...
            {
                cerr << "Missing argument to -c/--wmrconfig option\n";
                return EXIT_INVALID_CMDLINE;
            }
        }
        ...
        else
        {
            cerr << "Invalid argument: " << argv[argpos] << endl <<
                    ...
                    "-p or --port number        A port number to listen on (default is 6549) " << endl <<
                    "-d or --daemon             Runs mythwmrserver as a daemon " << endl <<
                    "-n or --nodaemon           Does not run mythwmrserver as a daemon (default)" << endl <<
                    "-c or --zmconfig           Location of wmr928's config file (default is " << WMR_CONFIG << ")" << endl <<
    ...
    map<int, WMRServer*> serverList; // list of WMRServers

    // load the config 
    loadWMRConfig(wmrconfig);
            ...
            if (FD_ISSET(i, &read_fds)) 
            {
                ...
                if (i == listener) 
                {
                    ...
                    else 
                    {
                        ...
                        // create new WMRServer and add to map
                        WMRServer *server = new WMRServer(newfd, debug);
